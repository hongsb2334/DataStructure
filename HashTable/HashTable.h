...
private:
    ...
    struct Entry {
        Entry(const std::string& key, const std::string& value) : key(key), value(value), next(nullptr)
        {};
...
private:
    ...
    //소수로 하는 게 좋고, 가능하면 큰 수를 하는 게 좋음
    static const int bucketCount = 19;

    //각 버킷에 연결된 첫 번째 엔트리 주소
