...
public:
    ...
    bool AddFront(const T& value)
    {
        ...
        data[front] = value;
        //인덱스 반대로 돌릴때, 소코반에서 메뉴 반대로 돌렸던거 생각
        front = (front - 1 + bufferSize) % bufferSize;

        return true;
    }

    //덱의 앞에서 데이터 제거하는 함수(큐의 Deque)
...
private:
    ...
    //데이터 배열, 여기 bufferSize써주려면 bufferSize가 static이어야 함
    T data[capacity + 1] = {};
